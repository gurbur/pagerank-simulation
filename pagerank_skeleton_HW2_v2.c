...
int main(int argc, char** argv) {
    ...
    double global_l1_diff = 0.0;
		int is_ring = (strcmp(P.gtype, "ring") == 0);
    ...
    for (it = 0; it < P.maxit &&!converged; ++it) {
         ...
         */
				for (long long i = off; i < off + locN; i++) {
						if (outdeg[i] == 0.0)
								local_dang_mass += pagerank_global[i];
				}
         ...
         * TODO: PageRank 계산 반복
         */
				// from teleport (to solve cyclic structure) = (1.0 - damping_factor) / N
				// from dangling mass (to solve dangling node) = damping_factor * global_dang_mass / N
				double base_pagerank = (1.0 - d) * one_over_N + d * global_dang_mass * one_over_N;
				for (long long i = off; i < off + locN; i++) {
						double new_pagerank = base_pagerank;
						double sum = 0.0;
						if (is_ring) {
         ...
         */
				MPI_Allgatherv(pagerank_new_local_chunk, (int)locN, MPI_DOUBLE, pagerank_global, counts, displacements, MPI_DOUBLE, MPI_COMM_WORLD);
    }
